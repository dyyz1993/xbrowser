...
const BASE_URL = 'https://data.stats.gov.cn/';
const API_BASE = '/dg/website/publicrelease/web/external';
const ROOT_CID = 'c4d82af16c3d4f0cb4f09d4af7d5888e';
const PK = ['北京','天津','河北','山西','内蒙古','辽宁','吉林','黑龙江','上海','江苏','浙江','安徽','福建','江西','山东','河南','湖北','湖南','广东','广西','海南','重庆','四川','贵州','云南','西藏','陕西','甘肃','青海','宁夏','新疆','香港','澳门','台湾'];

const PROVINCE_CODES: Array<{text: string; value: string}> = [
...
function gp(ctx: CommandContext): import('../types').Page {
  const p = ctx.page;
...
async function getTreeNodes(page: import('../types').Page, pid: string): Promise<TreeNode[]> {
  const data = await apiCall(page, '/new/queryIndexTreeAsync?pid=' + pid + '&code=6') as Record<string, unknown>;
...
async function findIndicatorId(page: import('../types').Page, name: string): Promise<{cid: string; indicatorId: string} | null> {
  const rootNodes = await getTreeNodes(page, '');
  for (let i = 0; i < rootNodes.length; i++) {
    const childNodes = await getTreeNodes(page, rootNodes[i].id);
    for (let j = 0; j < childNodes.length; j++) {
      if (childNodes[j].name === name || childNodes[j].name.indexOf(name) >= 0) {
        const indicators = await apiCall(page, '/new/queryIndicatorsByCid?cid=' + childNodes[j].id + '&dt=&name=') as Record<string, unknown>;
        const indList = ((indicators && indicators.data && (indicators.data as Record<string, unknown>).list) || []) as Array<Record<string, unknown>>;
        if (indList.length > 0) {
          return { cid: childNodes[j].id, indicatorId: String(indList[0].ek_dp || indList[0]._id || indList[0].id) };
        }
      }
      const grandChildren = await getTreeNodes(page, childNodes[j].id);
      for (let k = 0; k < grandChildren.length; k++) {
        if (grandChildren[k].name === name || grandChildren[k].name.indexOf(name) >= 0) {
          const indicators2 = await apiCall(page, '/new/queryIndicatorsByCid?cid=' + grandChildren[k].id + '&dt=&name=') as Record<string, unknown>;
          const indList2 = ((indicators2 && indicators2.data && (indicators2.data as Record<string, unknown>).list) || []) as Array<Record<string, unknown>>;
...
async function fetchAllProvinces(page: import('../types').Page, indicatorName: string): Promise<ProvinceRow[]> {
  const found = await findIndicatorId(page, indicatorName);
  ...
  const result = await apiCall(page, '/getEsDataByCidAndDt', body) as Record<string, unknown>;
  ...
  if (Array.isArray(entries)) {
    for (let i = 0; i < entries.length; i++) {
      ...
      const provName = entry.name || entry.areaName || '';
      let matched = false;
      ...
      if (entry.data && typeof entry.data === 'object') {
        ...
        for (let k = 0; k < keys.length; k++) {
          const val = toN(String(entry.data[keys[k]]));
          if (val !== null) years[keys[k]] = val;
        }
      } else if (entry.values && typeof entry.values === 'object') {
        const keys2 = Object.keys(entry.values);
        for (let m = 0; m < keys2.length; m++) {
          const val2 = toN(String(entry.values[keys2[m]]));
...
export default function(xcli: XCLIAPI): void {
  const stats = xcli.createSite({ name: 'stats', url: 'https://data.stats.gov.cn/', description: '国家统计局 - 分省年度经济数据（新站 API 模式，数据查询接口待官方上线）', requiresLogin: false });
  ...
  stats.command('indicators', {
    ...
    handler: async function(_p, ctx) {
      const page = gp(ctx);
      const rootNodes = await getTreeNodes(page, '');
      const result: Array<{name: string; children: string[]}> = [];
      for (let i = 0; i < rootNodes.length; i++) {
        const childNodes = await getTreeNodes(page, rootNodes[i].id);
        for (let j = 0; j < childNodes.length; j++) {
  ...
  stats.command('gdp', {
    ...
    handler: async function(params, ctx) {
      ...
      const prov = await fetchAllProvinces(page, '地区生产总值');
  ...
  stats.command('retail', {
    ...
    handler: async function(params, ctx) {
      const page = gp(ctx);
      const p = params as { format: string };
      const prov = await fetchAllProvinces(page, '社会消费品零售总额');
      if (prov.length === 0) return fail('未提取到省份数据', ['请检查网络或指标名称']);
      if (p.format === 'csv') return ok(toCsv(prov) as unknown as ProvinceRow[], ['CSV，' + prov.length + ' 省份']);
  ...
  stats.command('query', {
    ...
    handler: async function(params, ctx) {
      const page = gp(ctx);
      const p = params as { indicator: string };
      const prov = await fetchAllProvinces(page, p.indicator);
      if (prov.length === 0) return fail('未提取到省份数据', ['请检查网络或指标名称']);
  ...
  stats.command('report', {
    ...
    handler: async function(params, ctx) {
      ...
      const pm = await import('path');
      const p = params as { output: string; title: string };
      const gdp = await ctx.storage.get('stats_gdp') as ProvinceRow[] | null;
      const ret = await ctx.storage.get('stats_retail') as ProvinceRow[] | null;
      if (!gdp && !ret) return fail('无缓存数据', ['请先运行 stats gdp 或 stats retail']);
      const html = genHTML(p.title, gdp, ret);
      const op = pm.resolve(p.output);
      const d = pm.dirname(op);
  ...
  stats.command('export', {
    ...
    handler: async function(params, ctx) {
      const fs = await import('fs');
      const pm = await import('path');
      const p = params as { format: string; output: string | undefined; indicator: string };
      const ds: Record<string, ProvinceRow[] | null> = { gdp: await ctx.storage.get('stats_gdp') as ProvinceRow[] | null, retail: await ctx.storage.get('stats_retail') as ProvinceRow[] | null };
      const keys = p.indicator === 'all' ? ['gdp', 'retail'] : [p.indicator];
      const results: Record<string, unknown> = {};
      for (let i = 0; i < keys.length; i++) {
        const d = ds[keys[i]]; if (!d || d.length === 0) continue;
        results[keys[i]] = p.format === 'csv' ? toCsv(d) : d;
      }
      if (Object.keys(results).length === 0) return fail('无缓存数据', ['请先运行 stats gdp 或 stats retail']);
      if (p.output) { const op = pm.resolve(p.output); fs.writeFileSync(op, JSON.stringify(results, null, 2), 'utf-8'); return ok({path: op}, ['导出: ' + op]); }
...
function genHTML(title: string, gdp: ProvinceRow[] | null, ret: ProvinceRow[] | null): string {
  const gj = JSON.stringify(gdp || []); const rj = JSON.stringify(ret || []);
