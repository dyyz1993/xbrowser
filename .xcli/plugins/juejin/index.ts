...
export default function (xcli: XCLIAPI): void {
  ...
  site.command('login', {
    ...
    handler: async (params, ctx) => {
      ...
      return {
        ...
        tips: [loggedIn ? '掘金登录成功' : '登录可能未完成，请检查页面'],
      };
    },
  });
  ...
  site.command('publish', {
    ...
    handler: async (params, ctx) => {
      ...
      const titleInput = page.locator(
        'input[placeholder*="输入文章标题"], input[class*="title-input"], input[name*="title"]'
      ).first();
      ...
      const editor = page.locator(
        'div[contenteditable="true"][class*="editor"], textarea[class*="editor"], div[class*="CodeMirror"], textarea[placeholder*="请输入"]'
      ).first();
  ...
  site.command('draft', {
    ...
    handler: async (params, ctx) => {
      ...
      const titleInput = page.locator(
        'input[placeholder*="输入文章标题"], input[class*="title-input"]'
      ).first();
      if (await titleInput.isVisible().catch(() => false)) {
        await titleInput.fill(params.title);
      }
