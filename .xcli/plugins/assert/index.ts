    ...
    handler: async (p, ctx) => {
      ...
      try {
        switch (p.type) {
          ...
          case 'css': { const element = page.locator(p.selector!).first(); actual = await element.evaluate((el: Element) => getComputedStyle(el).getPropertyValue(String(p.value || ''))); passed = actual === String(p.expected || ''); message = `CSS "${p.value}": "${actual}" ${passed ? '==' : '!='} "${p.expected}"`; break; }
