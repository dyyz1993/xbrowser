      ...
      for (let i = 0; i < p.steps.length; i++) {
        ...
        try {
          switch (step.action) {
            ...
            case 'screenshot': { const sp = step.value || `test-step-${i}.png`; const buf = await page.screenshot({}) as unknown as Buffer; const { writeFile } = await import('fs/promises'); await writeFile(sp, Buffer.from(buf)); message = `Screenshot saved to ${sp}`; break; }
