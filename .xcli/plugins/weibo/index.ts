...
export default function (xcli: XCLIAPI): void {
  ...
  weibo.command('draft', {
    ...
    handler: async (params) => {
      ...
      const result = saveDraft('weibo', params.text, params.title);
      return ok({ saved: true, id: result.id, path: result.path }, [`草稿已保存: ${result.id}`]);
    },
  });

  weibo.command('drafts', {
    ...
    handler: async () => {
      ...
      const drafts = listDrafts('weibo');
      return ok({ drafts }, [`${drafts.length} 条草稿`]);
    },
  });

  weibo.command('load-draft', {
    ...
    handler: async (params) => {
      ...
      const draft = loadDraft('weibo', params.id);
