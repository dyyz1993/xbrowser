...
// 解析 JSON 输出（处理 ANSI 代码）
function parseJsonOutput(text: string): EngineResult | null {
  try {
...
async function searchEngine(engine: string): Promise<EngineResult | null> {
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', async (code) => {
      if (code !== 0) {
        // 尝试从 stderr 中提取 JSON（有些错误输出可能包含部分结果）
        const result = parseJsonOutput(stderr);
        if (result && result.domainExtraction && result.domainExtraction.totalUrls > 0) {
          console.log(`⚠️  [${engine}] 搜索有警告但成功，提取到 ${result.domainExtraction.totalUrls} 个 URL`);
          // 保存结果到文件
      ...
      try {
        // 尝试从 stdout 解析
        let result = parseJsonOutput(stdout);
        ...
        if (!result || !result.domainExtraction) {
          console.error(`❌ [${engine}] 解析结果失败: 没有找到有效的 domainExtraction 数据`);
          console.error(`   stdout 前500字符: ${stdout.slice(0, 500)}`);
          resolve(null);
          return;
        }
