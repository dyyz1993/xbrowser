...
describe('Douyin Plugin', () => {
  ...
  describe('Command: video-stats', () => {
    let commandConfig: any;

    beforeEach(() => {
      commandConfig = registeredCommands.get('video-stats');
    });

    it('should be registered with correct parameters', () => {
      expect(commandConfig).toBeDefined();
      expect(commandConfig.description).toBe('获取单个作品数据（网络拦截）');
      expect(commandConfig.scope).toBe('browser');
    });

    it('should get video data from response', async () => {
      let responseHandler: ((res: any) => Promise<void>) | null = null;
      mockPage.on.mockImplementation((event: string, handler: any) => {
        if (event === 'response') {
          responseHandler = handler;
      ...
      const mockResponse = {
        ...
        json: async () => ({
          aweme_detail: {
            ...
            video: {
              play_addr: { url_list: ['http://play.com'] },
              cover: { url_list: ['http://cover.com'] },
              width: 1080,
              height: 1920,
              duration: 15000,
              bit_rate: [
                {
                  gear_name: '1080P',
                  quality_type: 2,
                  play_addr: { url_list: ['http://1080p.com'], data_size: 5000000 },
                },
              ],
            },
            author: {
            ...
            text_extra: [{ hashtag_name: '#funny' }, { hashtag_name: '#viral' }],
          },
        }),
      };
    ...
    it('should handle missing video data', async () => {
      ...
      expect(result.data).toBeNull();
      expect(result.tips).toContain('未获取到视频数据，可能需要登录或视频不存在');
    });
  });
