...
describe('query commands', () => {
  ...
  it('text command should return empty string when textContent is null', async () => {
    ...
    const result = (await textCommand.handler({ selector: '#empty' }, ctx as any)) as Record<string, unknown>;
  ...
  it('text command should get body text when no selector', async () => {
    const ctx = makeCtx();
    const result = (await textCommand.handler({}, ctx as any)) as Record<string, unknown>;
