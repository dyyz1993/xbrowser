...
import plugin from '../../.xcli/plugins/gettyimages/index.ts';
...
describe('gettyimages plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name gettyimages', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'gettyimages', url: 'https://www.gettyimages.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'business' }, {})).rejects.toThrow('需要浏览器页面');
  });

  it('should navigate to search URL', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    await handler({ query: 'business' }, ctx);
    expect(ctx.page.goto).toHaveBeenCalledWith(
      'https://www.gettyimages.com/search/2/image?phrase=business',
      expect.any(Object)
    );
  });
  ...
  it('should return results with query and engine', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    const result = await handler({ query: 'business' }, ctx);
    const data = (result as Record<string, unknown>).data as Record<string, unknown>;
    expect(data.query).toBe('business');
    expect(data.engine).toBe('gettyimages');
  });
