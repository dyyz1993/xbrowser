...
import plugin from '../../.xcli/plugins/deviantart/index.ts';
...
describe('deviantart plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name deviantart', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'deviantart', url: 'https://www.deviantart.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'anime' }, {})).rejects.toThrow('需要浏览器页面');
  });

  it('should navigate to search URL', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    await handler({ query: 'anime' }, ctx);
    expect(ctx.page.goto).toHaveBeenCalledWith(
      'https://www.deviantart.com/search?q=anime',
      expect.any(Object)
    );
  });
  ...
  it('should return results with query and engine', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    const result = await handler({ query: 'anime' }, ctx);
    const data = (result as Record<string, unknown>).data as Record<string, unknown>;
    expect(data.query).toBe('anime');
    expect(data.engine).toBe('deviantart');
  });
