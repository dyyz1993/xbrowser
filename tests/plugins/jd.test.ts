...
import plugin from '../../.xcli/plugins/jd/index.ts';
...
function makeCtx(overrides: Record<string, unknown> = {}) {
  return {
    page: {
      ...
      evaluate: vi.fn(() => [
        { title: 'product', thumbnailUrl: 'https://img.jd.com/img.jpg', sourceUrl: '//item.jd.com/1.html', width: 200, height: 200 },
      ]),
    },
    ...overrides,
  };
...
describe('jd plugin', () => {
  ...
  it('should create site with name jd', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'jd' })
    );
  });

  it('should create site not requiring login', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
  ...
  describe('search-image command', () => {
    it('should throw when no page available', async () => {
      ...
      await expect(handler({ query: 'phone' }, {})).rejects.toThrow('需要浏览器页面');
    });

    it('should navigate to JD search URL', async () => {
      const handler = getHandler('search-image');
      const ctx = makeCtx();
      await handler({ query: 'phone' }, ctx);
      expect(ctx.page.goto).toHaveBeenCalledWith(
        expect.stringContaining('https://search.jd.com/Search?keyword=phone'),
        expect.any(Object)
      );
    });
    ...
    it('should return search results with query and engine', async () => {
      const handler = getHandler('search-image');
      const ctx = makeCtx();
      const result = await handler({ query: 'phone' }, ctx);
      const data = (result as Record<string, unknown>).data as Record<string, unknown>;
      expect(data.query).toBe('phone');
      expect(data.engine).toBe('jd');
