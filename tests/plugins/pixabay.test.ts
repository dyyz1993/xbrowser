...
import plugin from '../../.xcli/plugins/pixabay/index.ts';
...
describe('pixabay plugin', () => {
  ...
  it('should create site with name pixabay', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'pixabay', url: 'https://pixabay.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'cat' }, {})).rejects.toThrow('需要浏览器页面');
  });

  it('should navigate to search URL', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    await handler({ query: 'cat' }, ctx);
  ...
  it('should return results with query and engine', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    const result = await handler({ query: 'cat' }, ctx);
    const data = (result as Record<string, unknown>).data as Record<string, unknown>;
    expect(data.query).toBe('cat');
    expect(data.engine).toBe('pixabay');
  });
