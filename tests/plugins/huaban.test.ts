...
import plugin from '../../.xcli/plugins/huaban/index.ts';
...
describe('huaban plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name huaban', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'huaban', url: 'https://huaban.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'design' }, {})).rejects.toThrow('需要浏览器页面');
  });
  ...
  it('should navigate to search URL', async () => {
    ...
    expect(ctx.page.goto).toHaveBeenCalledWith(
      'https://huaban.com/search?q=design',
      expect.any(Object)
    );
  });
  ...
  it('should return results with query and engine', async () => {
    ...
    expect(data.engine).toBe('huaban');
  });
