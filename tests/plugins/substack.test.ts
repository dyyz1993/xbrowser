...
import plugin from '../../.xcli/plugins/substack/index.ts';
...
describe('substack plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name substack', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ name: 'substack' }));
  });
  ...
  describe('search command', () => {
    ...
    it('should return results from evaluate', async () => {
      ...
      const page = createMockPage([
        { name: 'Tech Weekly', description: 'Weekly tech news', subscribers: '10k', link: 'https://techweekly.substack.com', image: '' },
      ]);
      const ctx = createMockCtx(page);
      const result = await handler({ query: 'tech', limit: 10 }, ctx) as Record<string, unknown>;
