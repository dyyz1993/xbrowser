...
import plugin from '../../.xcli/plugins/pexels/index.ts';
...
describe('pexels plugin', () => {
  ...
  it('should create site with name pexels', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'pexels', url: 'https://www.pexels.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'nature' }, {})).rejects.toThrow('需要浏览器页面');
  });
  ...
  it('should return results with query and engine', async () => {
    ...
    expect(data.engine).toBe('pexels');
  });
