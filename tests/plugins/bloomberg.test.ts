...
import plugin from '../../.xcli/plugins/bloomberg/index.ts';
...
function createMockPage(evaluateResult: unknown = []) {
  return {
    url: vi.fn(() => 'https://www.bloomberg.com'),
...
describe('bloomberg plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name bloomberg', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ name: 'bloomberg' }));
  });

  it('should create site with correct url', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://www.bloomberg.com' }));
  });
  ...
  describe('search command', () => {
    ...
    it('should throw when no page', async () => {
      ...
      await expect(handler({ query: 'Apple', limit: 10 }, ctx)).rejects.toThrow('需要浏览器页面');
    });

    it('should return results from evaluate', async () => {
      const handler = getHandler('search');
      const page = createMockPage([
        { title: 'Apple News', summary: 'Apple releases new product', date: '2026-06-30', link: 'https://www.bloomberg.com/news/articles/xxx', image: '' },
      ]);
      const ctx = createMockCtx(page);
      const result = await handler({ query: 'Apple', limit: 10 }, ctx) as Record<string, unknown>;
