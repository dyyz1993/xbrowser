...
import plugin from '../../.xcli/plugins/reuters/index.ts';
...
function createMockPage(evaluateResult: unknown = []) {
  return {
    url: vi.fn(() => 'https://www.reuters.com'),
...
describe('reuters plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name reuters', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ name: 'reuters' }));
  });

  it('should create site with correct url', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://www.reuters.com' }));
  });
  ...
  describe('search command', () => {
    ...
    it('should throw when no page', async () => {
      ...
      await expect(handler({ query: 'tech', limit: 10 }, ctx)).rejects.toThrow('需要浏览器页面');
    });

    it('should return results from evaluate', async () => {
      const handler = getHandler('search');
      const page = createMockPage([
        { title: 'AI News', summary: 'New AI developments', date: '2026-06-30', link: 'https://www.reuters.com/technology/ai-news', image: '' },
      ]);
