...
import plugin from '../../.xcli/plugins/unsplash/index.ts';
...
describe('unsplash plugin', () => {
  ...
  it('should create site with name unsplash', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'unsplash', url: 'https://unsplash.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: 'mountain' }, {})).rejects.toThrow('需要浏览器页面');
  });

  it('should navigate to search URL', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    await handler({ query: 'mountain' }, ctx);
