...
import plugin from '../../.xcli/plugins/facebook/index.ts';
...
function createMockPage(evaluateResult: unknown = []) {
  return {
    url: vi.fn(() => 'https://www.facebook.com'),
...
const ALL_COMMANDS = ['search-image', 'post', 'like', 'comment', 'share'];
...
describe('facebook plugin', () => {
  ...
  // ─── 注册测试 ───
  it('should create site with name facebook', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ name: 'facebook' }));
  });

  it('should create site with correct url', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://www.facebook.com' }));
  });
  ...
  describe('search-image command', () => {
    ...
    it('should return results from evaluate', async () => {
      ...
      const page = createMockPage([
        { title: 'img1', thumbnailUrl: 'https://fbcdn.net/1.jpg', sourceUrl: 'https://fb.com/1', width: 200, height: 200 },
      ]);
      ...
      expect(data.engine).toBe('facebook');
    });
  });
  ...
  describe('post command', () => {
    ...
    it('should return fail on error', async () => {
      ...
      const ctx = createMockCtx(page);
      const result = await handler({ text: 'Hello!' }, ctx) as Record<string, unknown>;
  ...
  describe('like command', () => {
    ...
    it('should throw when no page', async () => {
      ...
      const ctx = createMockCtx(undefined);
      await expect(handler({ postUrl: 'https://fb.com/1' }, ctx)).rejects.toThrow('需要浏览器页面');
    });
  });
  ...
  describe('comment command', () => {
    ...
    it('should throw when no page', async () => {
      ...
      const ctx = createMockCtx(undefined);
      await expect(handler({ postUrl: 'https://fb.com/1', text: 'Nice!' }, ctx)).rejects.toThrow('需要浏览器页面');
    });
  });
  ...
  describe('share command', () => {
    it('should have browser scope', () => {
      const cmd = mockSite.command.mock.calls.find((c: unknown[]) => c[0] === 'share');
      expect((cmd![1] as Record<string, unknown>).scope).toBe('browser');
    });

    it('should throw when no page', async () => {
      const handler = getHandler('share');
      const ctx = createMockCtx(undefined);
      await expect(handler({ postUrl: 'https://fb.com/1' }, ctx)).rejects.toThrow('需要浏览器页面');
    });
  });
