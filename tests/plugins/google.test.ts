...
import plugin from '../../.xcli/plugins/google/index.ts';
...
describe('google plugin', () => {
  ...
  // ─── 注册测试 ───
  it('should create site with name google', () => {
  ...
  describe('search-image command', () => {
    ...
    it('should navigate to google images search URL', async () => {
      const handler = getHandler('search-image');
      const page = createMockPage([]);
      await handler({ query: 'cute cats', limit: 10 }, { page });
      expect(page.goto).toHaveBeenCalledWith(
        expect.stringContaining('https://www.google.com/search?q=cute%20cats&tbm=isch'),
        expect.any(Object)
      );
    });
    ...
    it('should return results with query and engine', async () => {
      const handler = getHandler('search-image');
      const page = createMockPage([
        { title: 'Cat', thumbnailUrl: 'https://t.com/img.jpg', sourceUrl: '', originalUrl: 'https://t.com/img.jpg', width: 400, height: 300, format: 'jpg' },
      ]);
      const result = await handler({ query: 'cats', limit: 10 }, { page }) as Record<string, unknown>;
      const data = result.data as Record<string, unknown>;
      expect(data.query).toBe('cats');
      expect(data.engine).toBe('google-images');
    ...
    it('should append size filter to URL', async () => {
      ...
      expect(page.goto).toHaveBeenCalledWith(
        expect.stringContaining('tbs=isz:l'),
        expect.any(Object)
      );
    });

    it('should append color filter to URL', async () => {
      const handler = getHandler('search-image');
      const page = createMockPage([]);
      await handler({ query: 'cats', limit: 10, color: 'red' }, { page });
      expect(page.goto).toHaveBeenCalledWith(
        expect.stringContaining('isc:red'),
        expect.any(Object)
      );
    });
    ...
    it('should append type filter to URL', async () => {
      ...
      expect(page.goto).toHaveBeenCalledWith(
        expect.stringContaining('itp:photo'),
        expect.any(Object)
      );
    });
    ...
    it('should include count in tips', async () => {
      const handler = getHandler('search-image');
      const page = createMockPage([
        { title: 'Cat', thumbnailUrl: 'https://t.com/img.jpg', sourceUrl: '', originalUrl: '', width: 100, height: 100, format: 'jpg' },
      ]);
      const result = await handler({ query: 'cats', limit: 10 }, { page }) as Record<string, unknown>;
  ...
  describe('webmaster-config command', () => {
    ...
    it('should save config to storage', async () => {
      ...
      const result = await handler({ site: 'https://xbrowser.dev' }, { storage }) as Record<string, unknown>;
      expect(storage.set).toHaveBeenCalledWith('google_webmaster', { site: 'https://xbrowser.dev' });
      const data = result.data as Record<string, unknown>;
  ...
  describe('push-url command', () => {
    ...
    it('should return fail when no params provided', async () => {
      const handler = getHandler('push-url');
      const result = await handler({}, {}) as Record<string, unknown>;
      expect(result.success).toBe(false);
    });

    it('should return fail when neither sitemapUrl nor urls provided', async () => {
      const handler = getHandler('push-url');
      const result = await handler({}, {}) as Record<string, unknown>;
      expect(result.success).toBe(false);
