...
const mockXCLI = {
  createSite: vi.fn((_cfg: unknown) => mockSite),
};
...
vi.mock('../../.xcli/plugins/zhihu/index.js', () => {
  return {
    default: vi.fn((xcli: typeof mockXCLI) => {
      ...
      const commands = ['search', 'trending', 'question', 'answer', 'publish', 'draft', 'article', 'chat'];
      for (const name of commands) {
        site.command(name, {
          handler: async (args: Record<string, unknown>, ctx: Record<string, unknown>) => {
            if (!ctx.page) throw new Error('需要浏览器页面');
            const result = (await (ctx.page as unknown as { evaluate: (fn: unknown) => Promise<unknown> }).evaluate(() => ({}))) as Record<string, unknown>;
...
describe('zhihu plugin', () => {
  ...
  it('should register expected command names', () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['search', 'trending', 'question', 'answer', 'article', 'chat']));
  });
