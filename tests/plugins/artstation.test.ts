...
import plugin from '../../.xcli/plugins/artstation/index.ts';
...
describe('artstation plugin', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    plugin(mockXCLI as unknown as Parameters<typeof plugin>[0]);
  });

  it('should create site with name artstation', () => {
    expect(mockXCLI.createSite).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'artstation', url: 'https://www.artstation.com' })
    );
  });
  ...
  it('should throw when no page available', async () => {
    ...
    await expect(handler({ query: '3d' }, {})).rejects.toThrow('需要浏览器页面');
  });

  it('should navigate to search URL', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    await handler({ query: '3d' }, ctx);
    expect(ctx.page.goto).toHaveBeenCalledWith(
      expect.stringContaining('https://www.artstation.com/search?sort_by=relevance&q=3d'),
      expect.any(Object)
    );
  });
  ...
  it('should return results with query and engine', async () => {
    const handler = mockSite.command.mock.calls[0][1].handler;
    const ctx = makeCtx();
    const result = await handler({ query: '3d' }, ctx);
    const data = (result as Record<string, unknown>).data as Record<string, unknown>;
    expect(data.query).toBe('3d');
    expect(data.engine).toBe('artstation');
  });
